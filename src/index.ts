...
const controls: Array<ControlFormat> = ["text", "confirm", "password", "email", "phone", "number", "integer", "date", "uuid", "url", "multiselect", "select"];
const formatByControl: Record<ControlFormat, InputFormat> = {
    text: "text",
    password: "text",
    email: "email",
    phone: "phone",
...
const selectControls = ["multiselect", "select"] as const;
const questionTypeByControl: Record<ControlFormat, QuestionType> = {
    text: "input",
    password: "password",
    confirm: "confirm",
...
export function colorText(text: string, color: "yellow" | "gray" | "green" | "red"): string {
    const colors = {
        yellow: "\x1b[33m",
        gray: "\x1b[90m",
        green: "\x1b[32m",
...
*/
export const createSurveyToken = async (helloKey: string, surveyData: string): Promise<string> => {
...
*/
export const extractEmailsFromToken = (emailParts: string | undefined): Array<string> => {
    try {
...
*   Extracts the survey data, public key and hello message from the compressed token
*/
export const extractSurveyToken = (compressedToken: string): { surveyData: string, publicKey: string, hello: string, emails: Array<string> } => {
    try {
...
*/
function validateInput(type: string, input: string): boolean {
    switch (type) {
...
*   Validates the input based on the provided format
*/
const compressQuestion = (question: Question): string => {
    let output = `${question.name}|!${question.message || '_'}|!${question.default || '_'}|!${question.control}|!${question.choices ? question.choices.join(",") : '_'}`;
    return output;
}

const complressQuestions = (questions: Question[]): string => {
    return questions.map(compressQuestion).join(";;");
}

const parseQuestion = (input: string): Question => {
    const [name, message, defaultValue, control, choices] = input.split("|!");
    return { name, message: message == '_' ? '' : message, default: defaultValue == '_' ? '' : defaultValue, control: control as ControlFormat, choices: choices == '_' ? [] : choices?.split(",").map((s) => s.trim()) };
}

const parseQuestions = (input: string): Question[] => {
    return input.split(";;").map(parseQuestion);
}
export const parseMessageQuestion = (message: string | null | undefined): { message: string; defaultValue?: string } => {
    if (!message) return { message: '_' };
...
export const parseInputQuestion = (input: string): { message: string; varName: string, defaultValue?: string } => {
    const [varName, message] = input.split("|").map((s) => s.trim());
    const messageData = parseMessageQuestion(message);
    return { varName, ...messageData };
}

export const helloToKey = (hello: string): string => {
...
export const requestEmails = async (): Promise<false | Array<string>> => {
    try {
...
*/
export const toFill = async (token: string) => {
    try {
...
export const createSurveyByKeys = async (envKeys: Array<string>) => {
        ...
        const { helloMessage } = await inquirer.prompt([
            {
                ...
                validate: (input) => {
                    ...
                    if (!value) return "Hello message cannot be empty";
...
const addQuestion = async (questions: Array<Question>) => {
    try {
        const { question } = await inquirer.prompt([
            {
                ...
                validate: (input) => {
                    ...
                    if (!validKey) return "Invalid variable name. Use only letters, numbers and underscores";
                    if (questions.find(q => q.name === key)) return "Variable name already exists";
                    return true;
                }
            }
        ]);
        ...
        if (isSelectControl) {
            ...
            choices = options.split(",").map((s: string) => s.trim());
        }
...
export const compressAnswers = (answers: Record<string, any>): string => {
    return Object.entries(answers).map(([name, value]) => `${name}::${value}`).join(";;");
}

export const decompressAnswers = (answers: string): Record<string, any> => {
...
export const fillSurvey = async (token: string) => {
    try {
                        ...
                        const { newEnvOutput } = await inquirer.prompt([
                            {
                                ...
                                validate: async (input) => {
                                    ...
                                    if (await pathExists(input)) return "File already exists";
                                    return true;
                                },
                                required: true,
                            }
